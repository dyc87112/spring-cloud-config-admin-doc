...
- `配置中心`：基于Spring Cloud Config原声的配置中心服务端，配合SCCA中针对不同存储实现的扩展（增强对不同配置存储介质的控制能力）搭建。
...
<dependency>
    ...
    <artifactId>scca-plugin-config-server-git</artifactId>
...
<dependency>
    <groupId>com.didispace</groupId>
    <artifactId>scca-plugin-config-server-db</artifactId>
    <version>1.1.0-RELEASE</version>
</dependency>
```
